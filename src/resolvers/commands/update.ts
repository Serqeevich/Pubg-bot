...
const UpdateResolver: CommandResolver = async (client, message) => {
  ...
  await feedbackMessage.edit(
    EmbedSuccessMessage(
      ...
      `Обновляем роли <@${message.author.id}> <a:load:945638352055586826>`,
    ),
  );
  ...
  if (message?.member) {
    ...
    .setDescription(`[${updatedUser.pubgNickname}](https://pubg.op.gg/user/${updatedUser.pubgNickname}) <a:arrow:945647104074854410> <@${message.author.id}>`)
    .setThumbnail(thumbnailUpdateMedia)
    .addField(`${updatedUser.stats?.bestRank ? updatedUser.stats?.bestRank : 'Unranked'} ${updatedUser.stats?.subTier ? updatedUser.stats?.subTier : ''}`, `> <:Point:945665709399216148> Point: ${updatedUser.stats?.currentRankPoint ? updatedUser.stats?.currentRankPoint :'0' } <:Adr:934113837970505788> ADR: ${updatedUser.stats?.avgDamage  ? updatedUser.stats?.avgDamage :'0'} <:KD:942655934856048641> KD: ${updatedUser.stats?.kd ? updatedUser.stats?.kd :'0'}`)
    ...
    .addField('** FPP Squad **', `\n> <:Adr:934113837970505788> ADR: ${updatedUser.stats?.adrFPP ? updatedUser.stats?.adrFPP : '0'}\n> <:KD:934114143500369920> KD: ${updatedUser.stats?.kdFPP ?updatedUser.stats?.kdFPP :'0'}`, true)
    .setTimestamp()
    .setFooter('Введите команду !update для обновления статистики.',`https://cdn.discordapp.com/attachments/939806800679690260/945611614244208650/GOTOP.png`)
    )
  }
};
